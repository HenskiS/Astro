"""
BACKTEST 1-HOUR BREAKOUT STRATEGY (FAST VERSION)
=================================================
More aggressive exit parameters for faster in-and-out trading
Target: <2 day average hold time (vs 4+ days in standard version)
"""
import pandas as pd
import numpy as np
import pickle
...
print("="*100)
print("BACKTEST: 1-HOUR BREAKOUT STRATEGY (FAST)")
print("="*100)
print()

# Strategy Parameters (aggressive exits for fast trading)
INITIAL_CAPITAL = 500
RISK_PER_TRADE = 0.004  # 0.4%
MIN_CONFIDENCE = 0.65

# Emergency stop: 60 hours = 2.5 days (vs 5 days in standard)
EMERGENCY_STOP_HOURS = 60
EMERGENCY_STOP_LOSS_PCT = -0.03  # Tighter: -3% (vs -4% standard)

# Trailing stop (more aggressive for faster exits)
TRAILING_STOP_TRIGGER = 0.0015  # 0.15% (vs 0.25% standard) - trigger sooner
TRAILING_STOP_PCT = 0.75  # 75% (vs 60% standard) - tighter trail, exit sooner

# Ladder parameters (take profits earlier)
LADDER_LEVELS = [0.003, 0.006]  # 0.3%, 0.6% (vs 0.4%, 0.8% standard)
...
for hour_idx, date in enumerate(all_trading_hours):
    ...
    for pair in PAIRS:
        ...
        # Determine direction and target (closer target for faster exits)
        if breakout_high_prob > breakout_low_prob:
            direction = 'long'
            breakout_level = row['high_80h']
            target = breakout_level * 1.003  # 0.3% (vs 0.5% standard)
        else:
            direction = 'short'
            breakout_level = row['low_80h']
            target = breakout_level * 0.997  # 0.3% (vs 0.5% standard)
...
print()
print("="*100)
print("BACKTEST RESULTS (FAST VERSION)")
print("="*100)
print()
...
print("="*100)
print("COMPARISON: STANDARD vs FAST")
print("="*100)
print()
print("Standard 1H Strategy:")
print("  CAGR: 53.2%")
print("  Max DD: -0.9%")
print("  Win Rate: 96.4%")
...
print()
print(f"Fast 1H Strategy (4 pairs, {years:.1f} years):")
print(f"  CAGR: {cagr:.1%}")
...
trades_df.to_csv('backtest_1h_fast_results.csv', index=False)
print("Results saved to: backtest_1h_fast_results.csv")
print()
