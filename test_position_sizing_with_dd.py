"""
TEST POSITION SIZING WITH DRAWDOWN
===================================
Test how both returns AND drawdown scale with position sizing.
Critical for understanding risk/reward tradeoff.
"""
import pandas as pd
import numpy as np
import pickle
...
def strategy_trail_with_sizing_and_equity(predictions, raw_data, position_pct):
    """Trail after 1x target hit with configurable position sizing + equity curve"""
...
# ============================================================================
print("="*100)
print("TESTING POSITION SIZING WITH DRAWDOWN")
print("="*100)
print()
...
print("="*100)
print("POSITION SIZING WITH DRAWDOWN")
print("="*100)
print()
...
for pos_size in position_sizes:
    ...
    for name, preds in all_predictions:
        result = strategy_trail_with_sizing_and_equity(preds, all_raw_data, pos_size)
        if result:
    ...
    if len(year_results) > 0:
        ...
        avg_max_dd = np.mean([r['max_drawdown'] for r in year_results])
        worst_max_dd = min([r['max_drawdown'] for r in year_results])
        ...
        print(f"  Avg Return: {avg_return:>6.1%} | Win Rate: {avg_win_rate:>5.1%} | Avg Max DD: {avg_max_dd:>6.1%} | Worst DD: {worst_max_dd:>6.1%}")
    print()

print()
print("="*100)
print("SUMMARY: POSITION SIZING VS DRAWDOWN")
print("="*100)
print()

print(f"{'Size':<6} {'Return':>8} {'Win Rate':>10} {'Avg DD':>10} {'Worst DD':>10} {'Return/DD':>10}")
print("-" * 70)

for pos_size in position_sizes:
    if pos_size in results_by_size:
        r = results_by_size[pos_size]
        return_dd_ratio = abs(r['avg_return'] / r['avg_max_dd']) if r['avg_max_dd'] != 0 else 0
        print(f"{pos_size*100:>3.0f}%{'':<3} {r['avg_return']:>7.1%} {r['avg_win_rate']:>9.1%} {r['avg_max_dd']:>9.1%} {r['worst_max_dd']:>9.1%} {return_dd_ratio:>9.1f}")
...
print("Return/DD Ratio = How much return per unit of drawdown risk")
print("  - Higher is better (more return for same risk)")
print("  - 1.0+ is good, 2.0+ is excellent")
print()

if 0.01 in results_by_size and 0.05 in results_by_size:
    r1 = results_by_size[0.01]
    r5 = results_by_size[0.05]

    print("Comparison: 1% vs 5% sizing")
    print(f"  1% sizing: {r1['avg_return']:.1%} return, {r1['avg_max_dd']:.1%} avg DD")
    print(f"  5% sizing: {r5['avg_return']:.1%} return, {r5['avg_max_dd']:.1%} avg DD")
    print()
    print(f"  Returns increased: {r5['avg_return']/r1['avg_return']:.1f}x")
    print(f"  Drawdown increased: {abs(r5['avg_max_dd']/r1['avg_max_dd']):.1f}x")
    print()

print("Recommendation based on risk-adjusted returns:")
best_ratio = max([(pos_size, abs(r['avg_return'] / r['avg_max_dd'])) for pos_size, r in results_by_size.items()], key=lambda x: x[1])
print(f"  Best Return/DD ratio: {best_ratio[0]*100:.0f}% sizing (ratio: {best_ratio[1]:.2f})")
print()
