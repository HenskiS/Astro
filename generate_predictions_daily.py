"""
GENERATE PREDICTIONS - DAILY RETRAINING
========================================
Train models with daily retraining (every trading day).
...
NOTE: This generates ~3,650 prediction periods. For practical testing, we sample
every 5th day to reduce computation time (resulting in ~730 periods).
"""
import pandas as pd
import numpy as np
import os
...
print("="*100)
print("GENERATING MODEL PREDICTIONS - DAILY RETRAINING")
print("="*100)
print()
...
# Sample frequency (1 = every day, 5 = every 5th day, etc.)
SAMPLE_FREQUENCY = 5

print(f"IMPORTANT: Using {GAP_DAYS}-day gap between train and test")
print(f"Sampling every {SAMPLE_FREQUENCY} days for computational efficiency")
print("This ensures no leakage from 10-day forward-looking targets")
print()

# Generate daily periods (2016-2025)
# Train on last 2 years (shorter for daily to manage computation)
# Leave 10-day gap, test for 1 day
TEST_PERIODS = []
...
while current_date < end_date:
    ...
    # Only process every Nth day
    if day_num % SAMPLE_FREQUENCY != 0:
    ...
    # Train period: 2 years of data, ending 10 days before test start
    train_end = test_start - pd.Timedelta(days=GAP_DAYS + 1)
    train_start = train_end - pd.Timedelta(days=365*2)
    ...
    TEST_PERIODS.append({
        ...
        'name': test_start.strftime('%Y-%m-%d'),
...
print(f"Total days to process: {len(TEST_PERIODS)}")
...
gap_violations = 0
for period in TEST_PERIODS[:10]:  # Check first 10 as sample
    if period['gap_days'] < GAP_DAYS:
        print(f"  WARNING: {period['name']} has only {period['gap_days']}-day gap")
        gap_violations += 1

if gap_violations == 0:
    print(f"  VERIFIED: All periods have proper {GAP_DAYS}-day gap")
else:
...
def train_models_for_period(pair_data, train_start, train_end):
    ...
    if len(train_df) < 500:  # Lower threshold for daily (2 years = ~500 days)
        return None
    ...
    if y_high.sum() > 50:  # Lower threshold for shorter train period
...
# Further sample for testing (every 20th period = roughly monthly)
TEST_PERIODS = [p for i, p in enumerate(TEST_PERIODS) if i % 20 == 0]
print(f"Further sampling to {len(TEST_PERIODS)} periods (every 100th day for speed)")
print()
...
for period in TEST_PERIODS:
    ...
    if days_processed % 10 == 0:
        print(f"[{days_processed}/{len(TEST_PERIODS)}] Processing {period['name']}...")
...
# Save predictions
output_file = 'model_predictions_daily.pkl'
