"""
OPTIMIZE 15M STRATEGY PARAMETERS
=================================
Test different parameter combinations for 15m trading with 8 pairs.

Key issues to address:
1. Losers hold too long (198.9 hours = 8.3 days!)
2. Poor profit ratio (0.19:1)
3. Emergency stop not catching bad trades fast enough

Focus areas:
- Emergency stop: Test MUCH shorter periods (24-60 = 6h-15h)
- Trailing stop: Tighter triggers and trail %
- Confidence threshold: Higher to be more selective
"""
import pandas as pd
import numpy as np
import pickle
...
print("="*100)
print("15M PARAMETER OPTIMIZATION (8 PAIRS)")
print("="*100)
print()
...
# Parameters to test
PARAM_GRID = {
    'emergency_periods': [24, 36, 48, 60],  # 6h, 9h, 12h, 15h (MUCH shorter!)
    'trailing_trigger': [0.001, 0.0015, 0.002, 0.0025],  # 0.1% to 0.25%
    'trailing_pct': [0.60, 0.65, 0.70, 0.75],  # Tighter trailing
    'confidence': [0.65, 0.70, 0.75],  # Higher thresholds
    'ladder_levels': [[0.002, 0.004], [0.003, 0.006]]  # Two ladder configs
}

print("Parameter space:")
print(f"  Emergency stops: {PARAM_GRID['emergency_periods']} periods")
for p in PARAM_GRID['emergency_periods']:
    print(f"    {p} periods = {p * 0.25:.1f} hours")
...
print(f"  Ladders: {PARAM_GRID['ladder_levels']}")
print()
...
for emergency, trigger, trail_pct, conf, ladders in product(
    ...
    results.append({
        ...
        'emergency_hours': emergency * 0.25,
...
for idx, row in results_df.head(20).iterrows():
    ...
    print(f"Rank #{rank}:")
    print(f"  Emergency: {row['emergency_periods']:.0f} periods ({row['emergency_hours']:.1f}h)")
    ...
    print(f"  Avg Winner: {row['avg_winner']:+.2%} ({row['avg_winner_hold_hours']:.1f}h)")
    print(f"  Avg Loser: {row['avg_loser']:+.2%} ({row['avg_loser_hold_hours']:.1f}h)")
    print(f"  Profit Ratio: {row['profit_ratio']:.2f}:1")
    print()

print("="*100)
print("TOP 20 BY PROFIT RATIO (minimum 50% return)")
print("="*100)
print()

results_df_ratio = results_df[
    (results_df['profit_ratio'] > 0) &
    (results_df['total_return'] > 0.5)
].sort_values('profit_ratio', ascending=False)

for idx, row in results_df_ratio.head(20).iterrows():
    rank = results_df_ratio.index.get_loc(idx) + 1
    print(f"Rank #{rank}:")
    print(f"  Emergency: {row['emergency_periods']:.0f} periods ({row['emergency_hours']:.1f}h)")
    print(f"  Trailing: {row['trailing_trigger']:.4f} trigger, {row['trailing_pct']:.0%} trail")
    print(f"  Confidence: {row['confidence']:.2f}")
...
print("="*100)
print("BEST BALANCE: Top 10 by Return with Good Profit Ratio (>0.30)")
print("="*100)
print()

results_df_balanced = results_df[results_df['profit_ratio'] > 0.30].sort_values('total_return', ascending=False)

for idx, row in results_df_balanced.head(10).iterrows():
    rank = results_df_balanced.index.get_loc(idx) + 1
    print(f"Rank #{rank}:")
    print(f"  Emergency: {row['emergency_periods']:.0f} periods ({row['emergency_hours']:.1f}h)")
    print(f"  Trailing: {row['trailing_trigger']:.4f} trigger, {row['trailing_pct']:.0%} trail")
    print(f"  Confidence: {row['confidence']:.2f}")
    print(f"  Ladders: {row['ladder_levels']}")
    print(f"  Return: {row['total_return']:+.1%} | Profit Ratio: {row['profit_ratio']:.2f}:1")
    print(f"  Win Rate: {row['win_rate']:.1%} | Trades: {row['num_trades']:.0f}")
    print(f"  Hold times: {row['avg_winner_hold_hours']:.1f}h winners, {row['avg_loser_hold_hours']:.1f}h losers")
    print()

# Save results
print("Saving results...")
results_df.to_csv('optimize_15m_results.csv', index=False)
print("Results saved to: optimize_15m_results.csv")
print()
