"""
VOL_EXPANSION: MOMENTUM STRATEGY WITH DRAWDOWN TRACKING
=======================================================
Calculate drawdown metrics for vol_expansion strategy.
"""
import pandas as pd
import numpy as np
...
print("="*100)
print("VOL_EXPANSION: MOMENTUM STRATEGY - DRAWDOWN ANALYSIS")
print("="*100)
print()
...
def backtest_momentum_strategy(model, all_pairs_data, test_years, year_label):
    """
    Backtest momentum strategy during vol_expansion with drawdown tracking
    """
    ...
    max_dd_start_idx = equity_series[:max_dd_end_idx].idxmax()
...
if losing_result and winning_result:
    ...
    print("="*100)
    print()
    print(f"Losing years: {losing_result['total_return']:+.1%} (DD: {losing_result['max_drawdown']:.1%})")
    print(f"Winning year: {winning_result['total_return']:+.1%} (DD: {winning_result['max_drawdown']:.1%})")
    print(f"Edge: {edge:+.1%}")
    print()

    # Calculate return/DD ratio
    losing_ratio = abs(losing_result['total_return'] / losing_result['max_drawdown']) if losing_result['max_drawdown'] != 0 else 0
    winning_ratio = abs(winning_result['total_return'] / winning_result['max_drawdown']) if winning_result['max_drawdown'] != 0 else 0

    print(f"Losing years Return/DD ratio: {losing_ratio:.2f}")
    print(f"Winning year Return/DD ratio: {winning_ratio:.2f}")
    print()
