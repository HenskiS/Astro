"""
STAGED PARAMETER OPTIMIZATION
==============================
Optimize parameters one at a time to narrow down the search space.

Stage 1: Optimize confidence threshold and risk per trade
Stage 2: Optimize stop parameters using best from Stage 1
Stage 3: Optimize trailing stop using best from Stages 1-2
"""
import pandas as pd
import numpy as np
import pickle
...
print("="*100)
print("STAGED PARAMETER OPTIMIZATION")
print("="*100)
print()
...
# STAGE 1: Optimize confidence and risk
print("="*100)
print("STAGE 1: Optimizing MIN_CONFIDENCE and RISK_PER_TRADE")
print("="*100)
print()
...
stage1_grid = {
    ...
    'RISK_PER_TRADE': [0.002, 0.003, 0.004, 0.005, 0.006],
}

best_stage1_score = -999
best_stage1_params = None

combinations = [(c, r) for c in stage1_grid['MIN_CONFIDENCE'] for r in stage1_grid['RISK_PER_TRADE']]
print(f"Testing {len(combinations)} combinations...")
print()

for conf, risk in combinations:
    ...
    if score > best_stage1_score:
        ...
        best_stage1_params = {'MIN_CONFIDENCE': conf, 'RISK_PER_TRADE': risk}
        print(f"New best! Conf={conf:.2f}, Risk={risk:.3f}")
        print(f"  CAGRs: {[f'{c:.1%}' for c in cagrs]}")
        print(f"  Avg: {avg_cagr:.1%}, Std: {std_cagr:.1%}, Score: {score:.3f}")
        print()

print(f"Best Stage 1: {best_stage1_params}")
print()

# STAGE 2: Optimize stop parameters
print("="*100)
print("STAGE 2: Optimizing Emergency Stop Parameters")
print("="*100)
print()
...
stage2_grid = {
    'EMERGENCY_STOP_PERIODS': [16, 20, 24, 28, 32],
    'EMERGENCY_STOP_LOSS_PCT': [-0.025, -0.03, -0.04, -0.05],
}

best_stage2_score = -999
best_stage2_params = None

combinations = [(p, l) for p in stage2_grid['EMERGENCY_STOP_PERIODS'] for l in stage2_grid['EMERGENCY_STOP_LOSS_PCT']]
print(f"Testing {len(combinations)} combinations...")
print()

for periods, loss_pct in combinations:
    params = stage2_params.copy()
    params['EMERGENCY_STOP_PERIODS'] = periods
    params['EMERGENCY_STOP_LOSS_PCT'] = loss_pct
    ...
    if score > best_stage2_score:
        ...
        best_stage2_params = {'EMERGENCY_STOP_PERIODS': periods, 'EMERGENCY_STOP_LOSS_PCT': loss_pct}
        print(f"New best! Periods={periods}, Loss={loss_pct:.3f}")
        print(f"  Avg: {avg_cagr:.1%}, Std: {std_cagr:.1%}, Score: {score:.3f}")
        print()

print(f"Best Stage 2: {best_stage2_params}")
print()

# STAGE 3: Optimize trailing stop
print("="*100)
print("STAGE 3: Optimizing Trailing Stop Parameters")
print("="*100)
print()
...
stage3_grid = {
    'TRAILING_STOP_TRIGGER': [0.0006, 0.0008, 0.001, 0.0012, 0.0015],
    'TRAILING_STOP_PCT': [0.65, 0.70, 0.75, 0.80],
}

best_stage3_score = -999
best_stage3_params = None

combinations = [(t, p) for t in stage3_grid['TRAILING_STOP_TRIGGER'] for p in stage3_grid['TRAILING_STOP_PCT']]
print(f"Testing {len(combinations)} combinations...")
print()

for trigger, trail_pct in combinations:
    params = stage3_params.copy()
    params['TRAILING_STOP_TRIGGER'] = trigger
    params['TRAILING_STOP_PCT'] = trail_pct
    ...
    if score > best_stage3_score:
        best_stage3_score = score
        best_stage3_params = {'TRAILING_STOP_TRIGGER': trigger, 'TRAILING_STOP_PCT': trail_pct}
        best_cagrs = cagrs
        print(f"New best! Trigger={trigger:.4f}, Trail={trail_pct:.2f}")
        print(f"  CAGRs: {[f'{c:.1%}' for c in cagrs]}")
        print(f"  Avg: {avg_cagr:.1%}, Std: {std_cagr:.1%}, Score: {score:.3f}")
        print()
...
final_params.update(best_stage3_params)
