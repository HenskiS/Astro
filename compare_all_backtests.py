"""
BACKTEST COMPARISON: ALL THREE VALIDATION METHODS
==================================================
Runs the same backtest strategy on all three prediction sets to compare results.
"""
import pandas as pd
import numpy as np
import pickle
...
# Import backtest logic from the original script
INITIAL_CAPITAL = 500
...
def run_backtest(predictions, raw_data, name):
    ...
    if len(all_trading_periods) == 0:
        print("  ERROR: No trading periods found")
        return None
    ...
    print(f"  Period: {min_date.date()} to {max_date.date()} ({days} days, {years:.1f} years)")
...
print("="*100)
print("BACKTEST COMPARISON: ALL VALIDATION METHODS")
print("="*100)
...
results_original = run_backtest(original_preds, all_raw_data, "1. ORIGINAL (70/30 split)")
results_validated = run_backtest(validated_preds, all_raw_data, "2. STRICT TIME SPLIT (no retrain)")
results_walkforward = run_backtest(walkforward_preds, all_raw_data, "3. WALK-FORWARD (periodic retrain)")
...
if all([results_original, results_validated, results_walkforward]):
    print(f"{'Method':<35} {'CAGR':<15} {'Max DD':<15} {'Win Rate':<15} {'Trades':<10}")
    print("-" * 100)
    print(f"{'1. Original (70/30)':<35} {results_original['cagr']:>13.1%}  {results_original['max_dd']:>13.1%}  {results_original['win_rate']:>13.1%}  {results_original['total_trades']:>8,}")
    print(f"{'2. Strict Time Split':<35} {results_validated['cagr']:>13.1%}  {results_validated['max_dd']:>13.1%}  {results_validated['win_rate']:>13.1%}  {results_validated['total_trades']:>8,}")
    print(f"{'3. Walk-Forward (REALISTIC)':<35} {results_walkforward['cagr']:>13.1%}  {results_walkforward['max_dd']:>13.1%}  {results_walkforward['win_rate']:>13.1%}  {results_walkforward['total_trades']:>8,}")
    ...
    cagr_diff = abs(results_original['cagr'] - results_walkforward['cagr'])

    if cagr_diff < 0.20:
        print("[GOOD NEWS] Walk-forward CAGR is within 20% of original")
        print(f"  Original: {results_original['cagr']:.1%}")
        print(f"  Walk-forward: {results_walkforward['cagr']:.1%}")
        print(f"  Difference: {cagr_diff:.1%}")
        print()
        print("  Your original results appear realistic!")
        print("  The walk-forward test confirms the strategy works with periodic retraining.")
    else:
        print("[IMPORTANT] Walk-forward CAGR differs significantly from original")
        print(f"  Original: {results_original['cagr']:.1%}")
        print(f"  Walk-forward: {results_walkforward['cagr']:.1%}")
        print(f"  Difference: {cagr_diff:.1%}")
        print()
        print("  Use walk-forward CAGR for production expectations.")
        print("  The difference suggests market regime changes or model decay over time.")
